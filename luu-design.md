...
**The mock's half of a scripted run is a file too.** `--mock-script <file>` reads reply blocks
(`## reply` opens one, everything above the first is the file's header) and **cycles** them, so
`2 × n` blocks are `n` turns of *a call, then the reply that reads its result*, repeated for as
long as the script runs. `--mock-reply` cannot do that — it hands over one reply per model *call*
and repeats the last, which is exactly one tool call however long the script is — and until this
existed no rule over tool results could be run on any corpus, only tested. The phase is arithmetic
rather than luck: a turn costs `1 + (the calls it made)` model calls, and a scripted run makes no
planning call, so a list whose length is a multiple of the calls per turn stays aligned; a run that
drifted says so, as a turn with no step in its recording. `scripts/mock/grounded-tools.txt` is the
one rule C was measured on. `serve` takes its replies as it always did: a planning call shifts the
phase at the first job, and a cycle there would need the mock to know where a turn begins —
which is a concept the backend trait does not have. See
[`RECORD/2026-09-13.a-mock-that-calls-a-tool.completed.md`](RECORD/2026-09-13.a-mock-that-calls-a-tool.completed.md).
