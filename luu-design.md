...
- **What leaves the history**: **built, both rules, both off**. A `cat` capped at 8 KiB and a selected fragment are the same object to the window — a block of file bytes that earned its place in *one* turn and is charged to every turn after it. **Rule A, *do not re-inject*** (`--repeat-once`): a span is rendered by the **oldest** turn of the window that carries it and skipped by every turn after it, the turn being asked included. Oldest and not newest is the whole reason it is cheap — the block above the newest message stays byte-identical — and it is also what stops eviction orphaning a span, because ownership is only handed *forward*, to a turn that outlives the one giving it up. The owner is chosen inside the window and never over the session: an owner already below the floor is [`the-fold-fix-verified`](RECORD/2026-08-30.the-fold-fix-verified.completed.md)'s failure with a different mechanism. A fragment's identity is its **path and its bytes**, so the same span after an edit is a different span. **Rule B, *prune behind*** (`--prune-behind`): when the window no longer fits, the oldest turns stop quoting before any turn is dropped — a pruned turn keeps its question and its answer and renders `// [pruned] path:1-40 — 312 tokens, no longer quoted` where its code was. It is a rung **above** eviction rather than a policy beside it, spending the half a path can rebuild before the half nothing can, and it takes `--evict`'s target rather than a knob of its own. Both watermarks are monotone for the same reason: a history whose front moves is the one thing a prefix cache cannot survive. **The measurement moved the second rule out of the category it was ordered in.** Twenty grounded turns at `--select-tokens 1024`, four arms one flag apart: A sends **4.6% fewer** prompt tokens (125 408 → 119 607), takes the `code` bucket down **19.7%**, and lifts mean prefix reuse from **34.0% to 52.1%**; B sends **1.2% more** (126 943) and **evicts nothing at all** — turns 1–13 leave the window in both arms that do not prune and stay in both that do. So B is not the token win the proposal argued it would be. The history block is 91% quoted code because code is what *fits*, not because code is what is *extra*: take it out and the turns eviction had been dropping move back in, and they are not free. The last turn of the baseline carries **6 124** tokens of history and the last turn of the pruning arm carries **6 121** — the same block to within three tokens, holding **7 turns** in one and **20** in the other. Together they are the best arm on every axis: 3.0% fewer tokens, nothing evicted, **54.5%** reuse. The accounting had to follow or the budget would lie — a turn costs its stored count *less what the render drops*, and `Evicted.tokens` is now the difference of two windows rather than the sum of the items below the floor, which was an over-report the moment a span could outlive the turn that owned it. A cut and a prune are both **protocol** messages (`evicted`, `pruned`, protocol 6 / format 8), because a pruned turn is still in the transcript and a client that could not tell would draw a grounded-looking turn whose grounding is not in the prompt. **What neither rule has is a model**: whether a 7B reads a span eleven turns up as well as one in the current message, and whether it answers from a tombstone or calls `read_file` on the path in it, is unmeasured and is why both are off — the protocol and its filed predictions are [`RECORD/2026-09-07.can-a-model-read-a-tombstone.WIP.md`](RECORD/2026-09-07.can-a-model-read-a-tombstone.WIP.md). See [`RECORD/2026-09-07.a-span-is-rendered-once.completed.md`](RECORD/2026-09-07.a-span-is-rendered-once.completed.md), [`RECORD/2026-09-07.pruning-behind.completed.md`](RECORD/2026-09-07.pruning-behind.completed.md), and the runs in [`RECORD/runs/2026-09-07.repeat-once/`](RECORD/runs/2026-09-07.repeat-once/) and [`RECORD/runs/2026-09-07.pruning-behind/`](RECORD/runs/2026-09-07.pruning-behind/).
