...
| # | Machine | Memory for weights | ~Bandwidth | Ceiling at Q4 | Status as of 2026-09-04 |
| --- | --- | --- | --- | --- | --- |
...
| 6 | **BC-250** — Zen 2 + RDNA 2 | 16 GB GDDR6 | ~224 GB/s | 14b comfortable | Vulkan serving stack |
...
| **6 · BC-250** | **A third serving stack.** Neither Metal nor CUDA — llama.cpp over Vulkan on AMD hardware | Unblocked by OpenAI backend |
...
Now that Phase 1 (Size sweep, Container observation, Gate probe) and the OpenAI-compatible backend are complete, Phase 2 is unblocked:
...
2. **The third serving stack (Machine 6 - BC-250 Vulkan)**: Test whether `luu` makes any vendor assumptions (CUDA/Metal) and measure Vulkan inference stability.
3. **Native Linux confinement (Machine 4 - Linux RTX)**: Test native Landlock enforcement without a virtualization boundary.
