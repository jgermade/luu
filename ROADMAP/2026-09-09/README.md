...
| 5 | **A grammar for tool calls** — the parse reads the block correctly; what fails is that generation does not stop. Narrowed to tool calls alone, the plan block stays as it is. Item 4's run gives this row a number to argue from rather than a guess: 20% of fifteen one-shot prompts parsed cleanly. **The curl is done** (2026-09-14): `llama-server`'s own `/v1/chat/completions` takes a bare `grammar` field, validates it, and enforces it — no template, no branch. **The grammar itself is built and measured, and it does nothing** (2026-09-14): `agent_core::grammar::compile` produces a real GBNF for all five tools (found, along the way: GBNF rule names cannot contain `_`), accepted by the live server — but `root ::= prose | call`'s `prose` (`[^\x00]*`) is a superset of `call`'s own language, so the alternation is set-theoretically identical to `prose` alone. Run against the tool-call probe's fifteen prompts, every reply came back byte-identical to the unconstrained run: same 3 Parsed / 2 Drifted / 2 Continued-past-fence / 8 No-call | nothing for the schema question; a fix for the leak, or `--constrain schema` (the retry arm, unbuilt, which has no leak to have) | [`a-grammar-for-tool-calls`](../../RECORD/2026-09-06.a-grammar-for-tool-calls.WIP.md), [`the-bare-grammar-field`](../../RECORD/2026-09-14.the-bare-grammar-field.completed.md), [`the-alternation-that-was-not-one`](../../RECORD/2026-09-14.the-alternation-that-was-not-one.completed.md) |
