...
| 7 | ~**What a container start costs** — *the worker starts with the session* was chosen on the shape of the thing and not on a number. The `container` job already makes the same call on both sides and is where the number comes from~ **measured 2026-09-15 against a real `luu serve` and a real Docker Desktop on machine 1: twenty interleaved `POST /api/sessions` pairs, host vs. container posture — 55.2 ms mean host, 155.8 ms mean container, a ~100 ms cost. Cheap enough that eager start is not a real trade for the common case; confirmed against `docker events` that each call is a real container start and not a cached one** | nothing further | [`what-a-container-start-costs`](../../RECORD/2026-09-15.what-a-container-start-costs.completed.md), from [`a-session-picks-its-executor`](../../RECORD/2026-09-08.a-session-picks-its-executor.completed.md) §Still open |
...
gantt
    ...
    What a container start costs           :done, cost, after trace, 1d
...
- **Item 7 closed 2026-09-15, and the number came in small.** A container
  start over a host one costs ~100 ms on machine 1 with the image already
  built — nobody switching sessions notices a tenth of a second, which is the
  clearest evidence yet that *the worker starts with the session* was the
  right call regardless of the reason it was made for. See
  [`what-a-container-start-costs`](../../RECORD/2026-09-15.what-a-container-start-costs.completed.md).
