...
| 5 | **A grammar for tool calls** — the parse reads the block correctly; what fails is that generation does not stop. Narrowed to tool calls alone, the plan block stays as it is. Item 4's run gives this row a number to argue from rather than a guess: 20% of fifteen one-shot prompts parsed cleanly. **The curl is done** (2026-09-14): `llama-server`'s own `/v1/chat/completions` takes a bare `grammar` field as an undocumented extension, validates it, and enforces it — no template, no branch, against the same weights machine 1 already has. **Ollama's own `/v1/chat/completions` does not**: the field is silently ignored, confirmed rather than only implied by the field table below | nothing for the field question. What is open: **whether to run `llama-server` directly on machine 1 instead of through Ollama** — every record on this machine has used Ollama, and only the bare binary honours the field | [`a-grammar-for-tool-calls`](../../RECORD/2026-09-06.a-grammar-for-tool-calls.WIP.md), [`the-bare-grammar-field`](../../RECORD/2026-09-14.the-bare-grammar-field.completed.md), [`the-tool-call-probe-run`](../../RECORD/2026-09-14.the-tool-call-probe-run.completed.md) |
