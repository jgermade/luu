...
| **A second header** (2026-09-07) | Changing model without losing the conversation, and the hole underneath it. `resume` takes `{provider, model}`; where the destination differs from what the session's stream names, the stream gains a second `Header` — same `started_at`, because every `at_ms` is relative to the first — and the context is re-folded with the new counter. It lands on a plain resume that inherited a different destination too, which was happening silently. The drive that proved it found a live bug beside it: the page named the gate's job field twice (`job` **and** `task`), serde refused the frame as a duplicate, and **no approval from this UI had reached the server since the rename** — [`a-second-header`](../../RECORD/2026-09-07.a-second-header.completed.md) |
...
| 4 | **A grammar for tool calls** — the text parse reads the block correctly; what fails is that generation does not stop. **Narrowed on 2026-09-06: tool calls only, the plan block stays as it is.** Same-corpus, same-scorer 14B number closed 2026-09-15 at 80% clean-call against the 7B's 20%, and `run_command`'s recurring envelope collapse — seen 3 times across 3 machine/corpus combinations — closed the same day: its `{command, args}` pair was restructured to one `argv` array, measured at 0/24 collapses on a follow-up run. Whether the pattern generalizes was checked the same day: `edit_file` and `write_file`, never before probed for format, came back 8/8 clean — no other tool's schema shares the collision-prone shape. The fix also holds on the 7B the collapse was first found on (0 collapses). The apparent backend gap that check surfaced (Drifted rate 3× higher on `llama-server` than the original Ollama run) turned out to be two different files sharing a model name, not a backend difference — closed by installing Ollama on machine 4 and comparing directly: same GGUF, either engine, byte-identical output; Ollama's own registry pull of the same model name is a different file and reproduces the original 20%. Whether the 14B has the same two-file problem was checked the same day: it does — a different-hash, same-label registry blob — but the quality direction reverses: 93% clean-call against the local file's 80%, where the 7B went the other way (20% against 40%). Two files, one name confirmed on both sizes; which one is better is not a property of that fact. The `--constrain schema` retry shape — unbuilt since 2026-09-14's own finding that the always-on arm never answers a prompt — was built the same day: an unconstrained first attempt, retried once under the schema only on a `Drifted` verdict. On the 7B, the model with drift to rescue, every one of six drifted prompts went from no tool touched to a correctly-shaped call executed (7/15 → 13/15 real first-attempt calls); the 14B, which already parses clean, never triggers the retry at all — a measured no-op, not a miss. A refused retry no longer loses the pre-retry answer (fixed the same day, by test rather than by reproduction — no server measured here has ever actually refused a compiled schema or grammar); it still fails silently rather than reporting once, loudly. The one hypothesized way a compiled constraint could fail — a zero-argument tool, llama.cpp #25923 — was built as a throwaway test tool and checked directly the same day: 200, not 400, on both arms, current llama-server build. Not a general claim about every schema-to-grammar converter, but the one case this file worried about does not reproduce here | nothing — `llama-server` is reachable through the OpenAI backend | [`a-grammar-for-tool-calls`](../../RECORD/2026-09-06.a-grammar-for-tool-calls.completed.md) |
...
| 15 | ~**A second header** — `POST /api/sessions/{id}/resume {provider, model}`: the same history, somewhere else, with a second `Header` line in the stream where the destination moved~ **closes item 14's own *Still open*: a resumed session already switched destination silently, and the line that fixes the switch fixes the silence. Found while driving it: every gate approval this repository's UI has sent since `d70e6d9` was dropped as a `duplicate field` and nobody had clicked Approve in a browser since** | nothing | [`a-second-header`](../../RECORD/2026-09-07.a-second-header.completed.md) |
...
gantt
    ...
    A second header (resume elsewhere)     :done, resume, 2026-09-07, 1d
