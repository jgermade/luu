...
**Grown once more on 2026-09-19, by three rows (23, 24, 25), and the last of
them is a sentence this file had been carrying rather than ordering.** *Nothing
counts how often any of this fires* was written five times across five files in
one revision, each time as an aside at the foot of a record about something
else, and each time it named one more uncounted thing than the time before. It
became row 25 on the day the count that *did* exist — item 24's, for one finding
— made the shape obvious: the failure was never that nothing counted, it was
that the first thing to count had nowhere to live and the second would have
gone beside it. Worth leaving visible as a calibration note, because it is the
opposite failure to the debug-UI fortnight below: that was work nobody ordered,
this was an order nobody wrote down.
...
**Grown a third time on 2026-09-21, by one row (26), and it is the first row this
file has taken straight from the *Still open* of the record that closed the row
before it.** Item 22's record named the draft's floor, argued for it, priced it
at an afternoon and declined to take it in the same commit; four days later that
is exactly what it cost. The calibration note is not about the estimate, which
was right. It is that **the row was in a record's closing section and not in this
file for a day**, which is the one place the split between a record and a roadmap
leaks: a record's *Still open* is where the next rows are born, and nothing moves
them here but somebody reading it. Row 26 also closed a defect nobody had
ordered — a resume that handed an approved job the policy file instead of its
plan — which is the third revision running in which **driving a surface found
what ordering it did not**.
...
| 19 | ~**A span is read once and never re-read, so a live session already sends bytes that a later turn edited away** — `code_context` is written at `push_turn_with_steps` and nothing refreshes it. Found while arguing about what a resume should store; independent of all of it, and nothing in the tree detects it. It is not a stale block sitting behind a fresh one: one prompt carries the same path **twice, with different bodies**, and rule A cannot collapse them because a fragment's identity is its path *and* its bytes, so the rule's own promise, *a span is rendered once*, silently does not hold for exactly the spans that changed~ **closed 2026-09-20, five days after the detector and one after the number.** The newest body of a path wins and the older ones go out as `// path — N tokens, superseded`; `record::FORMAT` 14 → 15 for the line that reports it. **The row's own blocker was retired by reading a field nobody had thought to read for this question**: it said a run against a model was needed because the corpus's edits were scripted, and that is true of *whether a model minds* and was never true of *what the repair costs*. The cost is a prefix number and `prefix_reuse` has been on the wire since long before any of this. Measured against the parent commit, same corpus and same flags: session prefix reuse **93.95% → 87.14%**, and **every point of it in turns 6 and 12** — the two turns that re-read the edited file — with the other ten inside 0.3 points. **The break is one per edit, not one per render**, because the citation standing in the old block's place is itself stable, which is the ratchet seen from the prefix's side. Two things the argument had backwards: it feared spending rule A's +18 points on every render, and it assumed the repair costs tokens — the history bucket **fell** 7 200 → 6 432, since a citation is cheaper than bytes nobody could trust. The ratchet turned out not to be a stored field either: the current body is the one the newest turn that read it carries, derived over every turn including the evicted ones, so it only moves forward because turns are only ever appended — and a file edited **back** to what it was is the case that makes *readers* the right thing to ratchet over rather than *bodies*. Unguarded where rule B's citation is guarded, because this one is a correction and not a saving. No new flag: the way back is `--no-repeat-once`, which is where the 8-of-13 numbers now live | the model run is item 5's bar and stays open; the overlapping-spec blind spot is inherited unchanged | [`the-newest-body-wins`](../../RECORD/2026-09-20.the-newest-body-wins.completed.md), from [`one-path-two-bodies`](../../RECORD/2026-09-19.one-path-two-bodies.completed.md) |
...
| 24 | ~**A corpus that edits a file and then asks about it again** — every script in `scripts/tasks/` either never writes (`grounded.txt`, twenty questions over a tree nothing touches) or writes without re-reading (`tool-call-probe-writes.txt`, eight one-shot calls scored on the call and not on what comes after). So **no corpus here can produce the one case where a span needs re-reading**, which is why item 19's defect survived every run this project has made and why rule A's default flipped on evidence that could not contain it. Needed before item 19's fix can be decided and before item 20 can be measured at all; the detector landed first so that there is something to count when it exists~ **landed 2026-09-19: `scripts/tasks/edit-reread.txt`, thirteen turns over a committed scratch tree, run in both arms against the mock.** Four files with four jobs — the case, a control that must stay quiet, and the two blind spots item 19's record could only name — and the number it produced is not the shape that record assumed: **two edits left 8 of 13 prompts carrying two bodies for one path**, and six of the eight attach nothing of that file. A divergence is a state the window enters, not an event at the turn that causes it. The `--select-tokens` arm doubled the lines without moving the renders, on a span nobody typed. It also cost something worth reading: a `src/lib.rs` in the fixture moved *this* repository's own selector probe by two first-place hits, so the tree is four leaves and a test says why | **the run against a model, which is Section A work** — the mock makes the number a fact about the corpus | [`a-corpus-that-edits`](../../RECORD/2026-09-19.a-corpus-that-edits.completed.md), for [`one-path-two-bodies`](../../RECORD/2026-09-19.one-path-two-bodies.completed.md) §What is built and what is not |
| 25 | ~**Nothing counts how often any of this fires** — not the three window rules, not the divergence, not item 12's grants, not the `not_granted` refusal. Written five times in five files across one revision and never as a row, which is how it kept being the thing somebody would get to~ **landed 2026-09-19, and the sentence had already stopped being true.** `record::divergence` arrived with item 24 the day before, and its own doc comment is the thesis for one finding — *a tally computed in a test is a tally only that test can quote*. So the gap was never *nothing counts*: it was that **one finding had a tally and four did not**, and that a `prunes()` beside a `divergence()` is the per-call reporter one abstraction higher. `SessionView::counts()` is **derived and never emitted** — arithmetic over lines the session already put on the wire, so it cannot drift from what it counts, which is `luu export`'s own founding argument one structure along — and it reaches the three surfaces every read-side thing here reaches: `luu count <file>`, `GET /api/sessions/{id}/counts`, and `counts.json` in an export. `divergence()` is **replaced rather than joined**, its four numbers unchanged, the substrate moving from `&[RecordLine]` to the fold. Three things it needed that the sentence did not predict: the fold had **written the bill down for itself** — refusals dropped under a comment ending *"it stays out of the view until something wants to count them"*, second time in two days a bill was found already itemised — so keeping them is additive and takes no format bump; **rule A had to be given a line first** (`repeated`, `record::FORMAT` 13 → 14), because the default was the one window rule reporting nothing at all, its number computed in `split_shown` and subtracted from a bucket, and a difference is not a measurement of what made it; and a **token total has to name every counter that produced part of it**, because a session resumed onto another model changes its counter mid-session and the first thing that sums is the first thing that could lose that silently. What the first run found, on a corpus built for another question: rule A collapses **7 spans with fragments typed and 43 with a selector on**, so most of the default's value is a fact about the *selector* — and `repeated` is 7 where `diverged` is 8 in the same session, which is item 19's mechanism as two integers rather than three paragraphs | nothing | [`one-counting-surface`](../../RECORD/2026-09-19.one-counting-surface.completed.md), run in [`runs/2026-09-19.one-counting-surface`](../../RECORD/runs/2026-09-19.one-counting-surface/) |
...
and a fifth, row 26, grown on 2026-09-21 out of what row 22 left behind.
...
| 11 | ~**What a container start costs** — *the worker starts with the session* was chosen on the shape of the thing and not on a number. The `container` job already makes the same call on both sides and is where the number comes from~ **measured 2026-09-15, two days before this revision was cut, on a branch that did not land until 2026-09-21** — so this row was carried already answered, and [`ROADMAP/2026-09-21`](../2026-09-21/README.md) carried it again. Struck here when [`ROADMAP/2026-09-23`](../2026-09-23/README.md) was cut | nothing | [`what-a-container-start-costs`](../../RECORD/2026-09-15.what-a-container-start-costs.completed.md), from [`a-session-picks-its-executor`](../../RECORD/2026-09-08.a-session-picks-its-executor.completed.md) §Still open |
| 12 | **Nothing counts how often a plan grants a file rather than a directory**, which is the number that would say how ordinary the 2026-09-08 regression was — and therefore how much of the gate's surface has no test. **Narrowed 2026-09-19 and deliberately not closed**: item 25's tally counts what every approved plan granted, reads against writes, and counts *path strings* — because which of `src/` and `src/lib.rs` is a directory is a question about a **disk**, the sandbox answers it with `is_dir()` when it resolves a root at approval, and a fold running on another machine six weeks later has no business asking. So the row's own noun is the part that is left: the classification is a field on `job_approved` written where the answer is known, which is the gate's to decide and not a counting surface's | the field, which is a format bump and a decision about what the gate writes down | [`the-surfaces-first`](../../RECORD/2026-09-08.the-surfaces-first.completed.md), narrowed in [`one-counting-surface`](../../RECORD/2026-09-19.one-counting-surface.completed.md) §What is rejected |
| 22 | ~**Every turn belongs to a job, and a session is an alternation** — `Turn::job` stops being `Option`: a session opens with a `draft`, `approve` closes it and opens a `plan`, `complete` closes that and opens the next draft, and the negative decisions (`decline`, `more changes`) keep you in the job you are in. Approving *is* closing, so a draft folds at approval and the exploratory turns that today can only be pruned or dropped gain the one mechanism they never had. `JobState::Rejected` moves from a state to a decision event inside the draft. ~**The largest item in this revision and the one with the least code written against it**~ **argued 2026-09-20, still the largest and now with a record under it**, which changes two parts of the shape by reading it against the code. `Turn::job` is not the `Option` that matters: `Job::plan` is, because **a draft is a job whose objective is known and whose plan is not**, and the empty plan is not *no restriction* but the strictest plan there is — so a draft carrying `Plan::default()` through `Plan::narrow` would refuse every exploratory turn its first `read_file`. It does not happen today only because the gate reads `Session::narrowed` rather than `turn.job`, and item 22 is the change that separates the two. So the `Option` **moves** — `Job::plan` gains one, `Turn::job` loses one — and the invariant is a sentence somebody can review against: *a draft is a job for the window and is not a job for the gate; the sandbox diff at the end of this work is nothing*. Also: `Proposed` joins `Rejected` in becoming a decision event inside the draft rather than a job state (approval is what hands out an id, so a refused plan never gets one), which leaves `JobState` at `Open`/`Closed`; and §eighth's zero-turn draft and `reopen_job`'s ambiguity in an alternation turn out to be **one** question, answered by *a job opens when a turn lands in it*, derived rather than stored~ **built 2026-09-20, and the protocol bump was the larger half exactly as the row said.** `Job::plan` is the `Option` and `Turn::job` is total; `JobState` is `Open`/`Closed`; the current job is derived; `protocol::VERSION` is **7** and `record::FORMAT` is **16**, with `job_proposed`/`job_rejected` kept parseable and never written again. **The invariant held: the sandbox diff is nothing** — no narrowing is built from a draft, and `Plan::narrow` is called at an approval and nowhere else. What the row could not predict is that **building it forced a decision neither §7th nor §8th had written down**: under an alternation there is always an open job, so `live_job().is_none()` stops being the trigger for the planning call and the gate has no door. It now has two, neither automatic — the model suggests a plan when a drafting turn's answer carries one, or a person asks with `request_plan` — and the planning call reads **the draft** rather than a held prompt, because *the plan is what the draft compacts to*. **No prompt is held any more**, which is the one behaviour a person can observe changing, and its cost is recorded rather than discovered: a drafting turn runs under the session's policy file, which on this repository grants `.` at `read-write` plus `cargo` and `git`, so exploration can now write where it used to wait behind the gate. Narrowing a draft to that floor minus the write bit is the row that falls out of this one | the protocol bump was the larger half, and the gate's whole surface was touched by a change whose point is that the gate does not move | [`every-turn-belongs-to-a-job`](../../RECORD/2026-09-20.every-turn-belongs-to-a-job.completed.md), from [`the-window-rules-are-a-session-fact`](../../RECORD/2026-09-18.the-window-rules-are-a-session-fact.completed.md) §7th, §8th |
| 26 | ~**The draft's floor** — item 22 made every turn belong to a job and gave a draft no plan, so a drafting turn fell through to the policy file: on this repository `.` at `read-write` plus `cargo`, `rustc` and `git`, which means exploration could edit the tree and run the build with nobody having approved anything. Named by item 22's own record as the obvious next row and deliberately not taken in the same change~ **landed 2026-09-21, and it is one expression**: `serve`'s turn falls back to `Sandbox::read_only` rather than to the policy file, so every site that clears the narrowing means the floor without being edited. Commands, network and enforcement are untouched — what a command may *do* is decided by the paths it may touch. **Three things the argument had not seen.** The floor first took `/dev/null` with it, which is granted read-write on purpose because `git` exits 128 before `main` without it — the implicit roots exist so a subprocess can *start* and were never part of what the agent may reach, and the record's own reasoning contained that distinction one sentence from where it was needed. Making `Agency::floor` private turned "remember to derive it" into a compile error and found **six** literals where a search had found four, under a doc comment that had said *two call sites resolving a policy differently is two sandboxes* since it was written. And the fallback it replaced turned out to be reached by one more path than drafts: **a session resumed inside an approved job was coming back held by the policy file rather than by its plan** — a silent widening, item 9's shape one field along, measured at rule `". (read)"` and now re-narrowed at the resume | nothing | [`the-drafts-floor`](../../RECORD/2026-09-21.the-drafts-floor.completed.md) |
...
gantt
    ...
    One surface that counts                :done, tally, 2026-09-19, 1d
    ...
    What a container start costs           :done, cost, after panel, 1d
