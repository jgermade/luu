...
async fn a_newer_client_is_refused_in_the_other_direction_too() {
    ...
    send(
        &mut socket,
        serde_json::json!({"type": "hello", "protocol": 8}),
    )
    .await;
...
async fn a_matching_client_is_greeted_and_then_ignored() {
    ...
    send(
        &mut socket,
        serde_json::json!({"type": "hello", "protocol": 7, "format": 9}),
    )
    .await;
